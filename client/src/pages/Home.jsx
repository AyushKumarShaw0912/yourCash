...
const Home = () => {
  ...
  const getTransactions = async () => {
    try {
      const res = await axios.get('http://localhost:8000/api/v1/expense', {
  ...
  const onDelete = async (id) => {
    try {
      const res = await axios.delete(`http://localhost:8000/api/v1/expense/delete/${id}`, {
        headers: { 'Content-Type': 'application/json' },
        withCredentials: true,
      });
      if (res.status === 200) {
  ...
  const handleSave = async (transaction) => {
    try {
      if (modalMode === 'edit') {
        ...
        await axios.put(`http://localhost:8000/api/v1/expense/update/${transaction._id}`, transaction, {
          headers: { 'Content-Type': 'application/json' },
          withCredentials: true,
        });
        ...
        // Add new transaction
        await axios.post('http://localhost:8000/api/v1/expense', transaction, {
          headers: { 'Content-Type': 'application/json' },
          withCredentials: true,
        });
